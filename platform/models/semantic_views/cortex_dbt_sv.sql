/*
  cortex_dbt_sv
  =============
  Semantic view pointing at dbt-governed mart tables (Act 3: "Trust").
  Every problem from Act 1 is solved:
...
TABLES (
    sales_orders  AS {{ ref('fct_sales_orders') }}
    ...
    b2b_orders    AS {{ ref('fct_b2b_orders') }}
      PRIMARY KEY (order_id)
      COMMENT 'B2B orders with clean revenue amount.',

    b2b_customers AS {{ ref('dim_b2b_customers') }}
